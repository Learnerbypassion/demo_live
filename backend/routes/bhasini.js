...
const AUDIO_CACHE_CANDIDATES = [
  process.env.AUDIO_CACHE_DIR,
  path.resolve(__dirname, "../../audio_cache"),
  path.resolve(__dirname, "../../frontend/public/audio_cache"),
  path.resolve(__dirname, "../audio_cache"),
].filter(Boolean);

const PRIMARY_AUDIO_CACHE_DIR = process.env.AUDIO_CACHE_DIR || path.resolve(__dirname, "../../audio_cache");
if (!fs.existsSync(PRIMARY_AUDIO_CACHE_DIR)) {
...
function getCachedAudio(text, lang, speaker = "") {
  const key = `${getCacheKey(text, lang, speaker)}.json`;
  for (const dir of AUDIO_CACHE_CANDIDATES) {
    try {
...
function setCachedAudio(text, lang, speaker = "", data) {
  try {
    const filePath = path.join(PRIMARY_AUDIO_CACHE_DIR, `${getCacheKey(text, lang, speaker)}.json`);
