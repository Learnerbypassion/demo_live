      ...
      <header className="bg-gradient-to-r from-brand-900 via-brand-950 to-brand-900 text-white shadow-md shrink-0 border-b border-brand-800">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 sm:h-18 flex items-center justify-between py-2 sm:py-3">
          <div className="flex items-center space-x-2.5 sm:space-x-3 min-w-0">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white/10 rounded-xl flex items-center justify-center p-1 sm:p-1.5 backdrop-blur-sm border border-white/20 shrink-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center space-x-1.5 sm:space-x-2">
                <span className="text-lg sm:text-xl font-black tracking-tight text-white">Dhanvantri</span>
                <span className="bg-emerald-500/20 text-emerald-300 text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full border border-emerald-400/30 uppercase tracking-wide">
                  ABDM Patient Portal
                </span>
              </div>
              <p className="text-[11px] sm:text-xs text-brand-200 hidden sm:block">National Centralized Digital Health & ABHA Gateway</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 shrink-0">
            ...
            <button
              ...
              className="flex items-center px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-semibold transition border border-white/10"
            >
              <LogOut className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1 sm:mr-1.5" /> Logout
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8 flex flex-col space-y-4 sm:space-y-6">
          ...
          <div className="bg-gradient-to-r from-brand-900 via-indigo-900 to-brand-800 p-4 sm:p-6 text-white relative">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 relative z-10">
              <div className="flex items-center space-x-3.5 sm:space-x-5 min-w-0">
                <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl bg-gradient-to-tr from-brand-500 to-indigo-600 flex items-center justify-center text-xl sm:text-2xl font-black text-white shadow-lg border-2 border-white/30 shrink-0">
                  {patient?.name ? patient.name.slice(0, 2).toUpperCase() : 'PT'}
                </div>
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-1.5 sm:gap-3">
                    <h1 className="text-xl sm:text-3xl font-bold text-white truncate">{patient?.name || 'Verified Patient'}</h1>
                    <span className="bg-emerald-500/20 text-emerald-300 border border-emerald-400/40 text-[10px] sm:text-xs px-2 py-0.5 rounded-full font-bold flex items-center space-x-1">
                      <CheckCircle2 className="w-3 sm:w-3.5 h-3 sm:h-3.5 mr-1" />
                      <span>ABHA Linked</span>
                    </span>
                  </div>
                  <div className="flex flex-wrap items-center gap-y-1 gap-x-2.5 sm:gap-x-4 text-[11px] sm:text-xs text-brand-100 mt-1.5 sm:mt-2">
                    <span>{patient?.age ? `${patient.age} yrs` : 'Age N/A'}</span>
                    <span>•</span>
                    <span>{patient?.gender || 'Patient'}</span>
                    <span>•</span>
                    <span>+91 {patient?.phone || 'XXXXXXXXXX'}</span>
                    {patient?.blood_group && (
                      <>
                        <span>•</span>
                        <span className="bg-rose-500/20 text-rose-200 px-1.5 py-0.5 rounded font-bold">Blood: {patient.blood_group}</span>
                      </>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-black/30 backdrop-blur-md border border-white/15 p-3.5 sm:p-4 rounded-xl flex flex-col space-y-2 w-full md:w-auto md:min-w-[280px]">
                ...
                <div className="flex items-center justify-between bg-white/10 px-3 py-1.5 rounded-lg border border-white/10">
                  <span className="font-mono font-bold text-sm sm:text-base tracking-wider text-white select-all truncate">
                    {activeAbhaId}
                  </span>
                  <button
                    onClick={handleCopyAbha}
                    className="ml-2 p-1.5 text-white/70 hover:text-white rounded hover:bg-white/10 transition shrink-0"
                    title="Copy ABHA ID"
                  >
                ...
                <div className="flex items-center justify-between text-[11px] text-gray-300">
                  <span className="truncate">Address: {activeAbhaId ? `${activeAbhaId.replace(/[^0-9]/g, '')}@abdm` : 'Not registered'}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-gray-100 border-b border-gray-200 bg-gray-50/50">
            <div className="p-3 sm:p-5 text-center">
              ...
              <div className="flex items-center justify-center space-x-1.5">
                ...
                <span className="text-xl sm:text-2xl font-black text-gray-900">{abhaRecords.length}</span>
              </div>
            </div>
            <div className="p-3 sm:p-5 text-center">
              <p className="text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Hospitals Visited</p>
              <div className="flex items-center justify-center space-x-1.5">
                <Building2 className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-600" />
                <span className="text-xl sm:text-2xl font-black text-gray-900">{uniqueHospitals || (abhaRecords.length > 0 ? 1 : 0)}</span>
              </div>
            </div>
            <div className="p-3 sm:p-5 text-center">
              <p className="text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Latest Diagnosis</p>
              <p className="text-xs sm:text-sm font-bold text-gray-800 truncate px-1 sm:px-2" title={cleanLatestDiagnosis}>
                {cleanLatestDiagnosis}
              </p>
            </div>
            <div className="p-3 sm:p-5 text-center">
              <p className="text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">ABDM Consent</p>
              <span className="inline-flex items-center px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
                <Shield className="w-3 h-3 sm:w-3.5 sm:h-3.5 mr-1" /> DPDP Active
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-1 sm:space-x-2 border-b border-gray-200 overflow-x-auto pb-0 px-1">
          <button
            ...
            className={`pb-2.5 sm:pb-3 px-3 sm:px-5 font-bold text-xs sm:text-sm border-b-2 flex items-center space-x-1.5 sm:space-x-2 transition-all whitespace-nowrap ${
              activeTab === 'abha'
          ...
          >
            ...
            <span className="ml-1 px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-xs font-bold rounded-full bg-brand-100 text-brand-800">
              {abhaRecords.length}
            </span>
          </button>
