...
const API_BASE = import.meta.env.VITE_API_URL
  ...
  : (_isHttps
      ? '/api'
      : (!_isLocalhost && typeof window !== 'undefined' && window.location.hostname
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        const err = new Error(data?.message || data?.error || `Upload failed (HTTP ${xhr.status})`);
        err.code = data?.code;
...
export default function MobileUpload() {
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      if (err.code === 'LOCAL_OCR_UNAVAILABLE' || err.status === 503) {
        setUploaded(true);
        setProgress(100);
        setOcrNotice(true);
        return;
        ...
        <div style={cardStyle}>
          ...
          <div style={{ padding: '36px 24px 28px' }}>
            ...
            <p style={{
              ...
              Your document has been received and attached to your kiosk session.
            </p>
            ...
            {ocrNotice && (
              <div style={{
                ...
                <strong>On-Premise Privacy Notice:</strong> Prescription OCR runs locally on hospital hardware and is intentionally disabled in this public demonstration. Full OCR processing is demonstrated in our submission video.
              </div>
            )}
